...
| N4 | Partitioned queue (256, `hash(userId)`) + workers absorb the transition path; API goes 202/stateless; dedup state leaves the hot path | N4A complete — broker infrastructure only: consistent-hash topology, quorum partitions, DLQ with policy-set delivery-limit, immutable-count guard; routing and distribution proven (same user → same partition; 10k users max/min 1.09) — [ADR 0014](docs/ADR/0014-rabbitmq-topology.md). N4B (publish) and N4C (worker) not started |
