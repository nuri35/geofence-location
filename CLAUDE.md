...
| N6 | Load verification of the async pipeline with real numbers (the harness evolves with it) | First measurement complete (2026-08-09, ABBA, single box, 8 partitions) — API ingest ceiling 4.6–5.0k req/s static; one worker absorbs ~2.5–3.1k transitions/s and DB-stall timing decides backlog onset (worst episode 13.7k deep, API unaffected, drained ~14 s); e2e 202→log p50 ≤30 ms / p99 ≤130 ms healthy, p99 4.1–4.4 s under backlog. ADR 0011's absorption claim held; ADR 0018's ~99% stays a traffic assumption (mechanism proven: pool idle at full static load). **Open: worker scaling is untestable with workers on the API's box** — [docs/ASYNC_LOAD_MEASUREMENT.md](docs/ASYNC_LOAD_MEASUREMENT.md) |
