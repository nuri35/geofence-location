...
| Touch presence memory, or anything near partition ownership | `docs/ADR/0018-worker-local-presence.md` — memory is worker-only, seeded from Postgres only, and correct ONLY under exclusive partition ownership; any change that could let a second consumer serve a partition must invalidate worker memory first |
